				...
				if (cosmetics.containsKey(Slot.CAPE)) {
					...
					RemoteAssets.getInstance().getRemoteMesh(cosmetic.seq(), cosmetic.fileLocation(), (mesh) -> {
						...
						if (cosmetic.useIntParam1() && (!cosmetic.useBoolParam1() || !cosmetic.boolParam1())) {
							...
							int hueColor = ColorUtils.hue(hue);
							int saturationApplied = ColorUtils.interpolate(saturation, hueColor, 0xFFFFFFFF);
							int brightnessApplied = ColorUtils.interpolate(brightness, saturationApplied, 0xFF000000);
