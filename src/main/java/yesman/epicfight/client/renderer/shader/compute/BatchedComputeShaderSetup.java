...
public abstract class BatchedComputeShaderSetup extends ComputeShaderSetup{
    ...
    public BatchedComputeShaderSetup(SkinnedMesh skinnedMesh, int outBufferSize) {
        super(skinnedMesh, 1);
        if (POSES_DATA == null){
            int len = (int)Math.max(8192,
                    ComputeShaderLoader.getGLMaxSSBOSize() / (16 * 4));
        ...
        outVertexAttrBO.close();
        int max = (int) Math.max(ComputeShaderLoader.getGLMaxSSBOSize()/4, 1024 * 1024 * 1024 / 4);
        outVertexAttrBO = new OutputSSBO((short) outBufferSize, max, DynamicSSBO.DataMode.STREAM);
    }

    public static class ModelInfo{
        public int pose_offset = 0;
        public int hidden_offset = 0;
        ...
        public void store(FloatBuffer buf){
            buf.put(Float.intBitsToFloat(pose_offset));
            buf.put(Float.intBitsToFloat(hidden_offset));
        }
    }
}
